...
# =============================================================================
'''
In this file we define the functions that are required to perform document similarity with SentNet using a Doc2Vec model implimented in Gensim.
...
def read_corpus_sim(data, field, target, tokens_only=False):
    ...
    Purpose: This function prepares text for processing within the Gensim model by converting documents to the doc2vec.TaggedDocument class format. This involves:
                ...
                2) Storing a list tags associated with the list of terms. In this case we store the value (target) of the essay associated with the given word list.
...
def Doc2Vec_Sim_Estimates(model, train_corpus, limit=0):
    '''
    Purpose: This function produces an estimated score for every document by attempting to identify which group/cluster of documents it is the most similar to (based on the score of those documents).
             This requires the following steps:
                 
                 1) Initalize an empty estimates dataframe to hold estimated values in
                 2) For every document in the dataset find extract features from that document and compare it to the known features within the labeled training set
                 3) Find the document group/cluster this group is the most similar as well as the "probabiliy"/"similarity" of this document belonging to that group

...
def read_corpus(data, field, tokens_only=False):
    ...
    This involves:
        ...
        2) Storing a list tags associated with the list of terms. In this case 
            we store the index of the essay associated with this word list
   ...
   Input: The following inputs are required for this function:
        ...
        2) field = the column/feature within the dataframe that contains the 
            text to be analyzed
        3) tokens_only = If you would only like tokens produced for the document 
...
def Doc2Vec_Training_Model(train_corpus, field):
    ...
    can be used to score future documents on.
    '''
...
def Doc2Vec_Estimates_Training(model, train_corpus, scores_join, target, limit=0):
    ...
    attempting to identify the closest related/matching document to the target 
    document.
    ...
    Input: This function requires the following inputs:
        ...
        1) model = a gensim.models.doc2vec.Doc2Vec object (can be generated 
             using the Doc2Vec_Training_Model function)
    ...
    Output: This function returns a pandas DataFrame with the following features:
        ...
        2) Pred_Class = The predicted class/value of that document 
    ...
    for doc_id in range(len(train_corpus)):
        ...
        sims = pd.merge(
                sims, scores_join, on=['Pred_Class_Index','Pred_Class_Index'])
        ...
        estimates = estimates.append(
                {'Doc_ID':doc_id, 'Pred_Class':pred_class,\
...
def Doc2Vec_Estimates_Testing(model, train_corpus, scores_join, target, limit=0):
    ...
    Output: This function returns a pandas DataFrame with the following features:
        ...
        1) Doc_Id = The index of provided document
        2) Pred_Class = The predicted class/value of that document (assigned
    ...
    estimates = pd.DataFrame(
            columns=['Doc_ID','Pred_Class','Pred_Prob','Weighted_Class'])
    ...
    for doc_id in range(len(train_corpus)):
        ...
        # Multiply the probability of a document match by the score of that 
...
def Document_Matching_Training(train, doc, target, limit):
    ...
    Input: This function requires the following inputs:
        ...
        1) train = a pandas DataFrame that contains the documents to be 
        ...
        4) limit = the probability limit that a prediction must exceed for 
            that prediction to be assigned/passed on (usually .7, but defaults 
            to 0 if not assigned)
    ...
    Output: This function returns three items in a dictionary:
            ...
            3) gensim_model = the Doc2Vec gensim model developed using the 
    ...
    train_estimates = Doc2Vec_Estimates_Training(
            gensim_training, train_corpus, scores_join, target, limit=0)
    
    return({'train_estimates':train_estimates, 'scores_join':scores_join, \
            'gensim_model':gensim_training})
...
def Document_Matching_Testing(test, doc, target, scores_join, gensim_training, limit):
    ...
    to produce target estimates for every document in a provided testing set.
   
    It accomplishes this through the following steps/functions:
                 
        1) Develops a testing corpus for the Doc2Vec model using the read_
            corpus function
    ...
    Input: This function requires the following inputs:
        ...
        3) target = the column name (string) that contains the score we are 
            attempting to estimate
        4) gensim_training = the gensim.models.doc2vec.Doc2Vec model (provided 
             by the Document_Matching_Training function)
    ...
    # Develop the training corpus
    train_corpus = list(read_corpus(test, doc, target))
    
    # Obtain Estimates
    test_estimates = Doc2Vec_Estimates_Training(
            gensim_training, train_corpus, scores_join, target, limit=0)
